	...
	<ul class="space-y-4 rounded-lg bg-gray-100 p-4 shadow-md">
		...
		{:else}
			{#each data.flocks as flock}
				<li class="pop-in flex items-center justify-between rounded-lg bg-white p-4 shadow-sm">
					<span class="text-black-500">
						{flock.name || 'Unnamed Flock'} - Total Mortality: {flock.totalMortality}
					</span>
					<button
						class="rounded-sm bg-green-700 px-3 py-1 text-white hover:bg-green-600"
